...
def main():
    ...
    shoot_timer = 0.0
    shoot_interval = 0.15  # seconds between shots
    while running:
        dt = clock.tick(FPS) / 1000.0

        keys = pygame.key.get_pressed()
        # Determine shooting direction from arrow keys (can be diagonal)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_SPACE, pygame.K_w):
                    player.jump()
                # Arrow key pressed: update state
                if event.key in arrow_keys:
                    arrow_keys[event.key] = True
            if event.type == pygame.KEYUP:
                if event.key in (pygame.K_SPACE, pygame.K_w):
                    player.short_jump_cut()
                # Arrow key released: update state
                if event.key in arrow_keys:
                    arrow_keys[event.key] = False

        # Shoot continuously while arrow key(s) held
        if shoot_dir:
            shoot_timer += dt
        ...
        player.draw(screen)
        # Draw shooting direction arrow if any
        if shoot_dir:
            dx, dy = shoot_dir
            mag = (dx**2 + dy**2) ** 0.5
            if mag:
