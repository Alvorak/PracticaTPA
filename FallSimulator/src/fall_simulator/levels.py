...
class LevelSpec:
    ...
    def create_platforms(
        self, viewport_w: int, ground_y: int, PlatformFactory=None
    ):  # plataforma factory opcional (plataforma personalizada)
        ...
        for p in self.platforms:  # Itera sobre cada especificación de plataforma
            ...
            if "x_frac" in p:
                x = (
                    int(p["x_frac"] * viewport_w) - w // 2
                )  # Centrado en x_frac usando viewport_w para el ancho
            else:
                x = int(p.get("x", 0))
...
def load_levels(path: str = None) -> List[LevelSpec]:
    ...
    if path is None:
        path = (
            Path(__file__).parent / "data" / "levels.json"
        )  # Ruta por defecto de niveles
    else:
        path = Path(path)

    if not path.exists():
        raise FileNotFoundError(
            f"Archivo de niveles no encontrado: {path}"
        )  # Ruta no encontrada
